...
const News = () => {
    ...
    const saveFile = (file) => {
        try {
            if (file) {
                FileSaver.saveAs(
                    `${process.env.REACT_APP_ROOT_URL}/${file}`,
                    file.split(`\\`)?.at(-1)?.split(`_`)[0] + file?.split(`\\`)?.at(-1)?.split(`_`)[2]
                );
            }
        } catch (error) {
            ...
            <div>
                <div className="bg-gray-100" >
                    <section className='container mx-auto'>
                        {
                            data ? <section className="text-gray-600 body-font">
                                {data ? data.map((item, i) =>
                                    <div key={i} className="container px-5  mx-auto">
                                        ...
                                        <div id={createId(item.headline, i)} className="p-5 bg-white flex items-center mx-auto border-b my-1 border-gray-200 rounded-lg sm:flex-row flex-col">
                                            ...
                                            <div className="sm:w-36 mb-auto sm:h-32 h-20 w-20 sm:mr-6 inline-flex items-center justify-center flex-shrink-0">
                                                <a href={item.thumbnail ? `${process.env.REACT_APP_ROOT_URL}/${item?.thumbnail}` : null} target="_blank" download rel="noopener noreferrer" className='relative'>
                                                    <img
                                                        ...
                                                        src={item.thumbnail ? `${process.env.REACT_APP_ROOT_URL}/${item?.thumbnail}` : noImage} />
                                            ...
                                            <div className="flex-grow sm:text-left text-center mt-6 sm:mt-0">
                                                ...
                                                <div className='text-xs '>{item.links ? <div>
                                                    {
                                                        <ul className='flex flex-col text-blue-900 '>{(item.links)?.map((perlink, i) => <a
                                                            key={i} className='hover:underline visited:text-purple-600' target={'_blank'} href={perlink} rel="noreferrer">{perlink} </a>)}</ul>
                                                    }
                                                </div> : ''}</div>
                                                <div className="py-4">
                                                ...
                                                <div className="md:flex font-bold text-gray-800">
                                                    ...
                                                    <div className="w-full md:w-1/2 flex space-x-3">
                                                        ...
                                                        {
                                                            item.files ? <div className="w-full  flex ">
                                                                <div className="">
                                                                    <h2 className="text-gray-500 text-sm" >Files</h2>
                                                                    <ol className='flex space-x-3 text-xs'>
                                                                         
                                                                        {(item.files).map((fileDownload,i) => <li key={i} className='flex'>
                                                                            <span className="cursor-pointer font-normal inline-flex items-center rounded-full p-1 bg-green-500 text-white group transition-all duration-500  hover:scale-110   " role="alert" tabIndex="0">
                                                                                <MdDownload className='text-lg'></MdDownload>

                                                                                <span onClick={e => saveFile(fileDownload)} className="whitespace-nowrap inline-block group-hover:max-w-screen-2xl group-focus:max-w-screen-2xl max-w-0 scale-80 group-hover:scale-100 overflow-hidden transition-all duration-500 group-hover:px-2 group-focus:px-2">{fileDownload.split(`\\`).at(-1).split(`_`)[0] + fileDownload.split(`\\`).at(-1).split(`_`)[2]}</span>
                                                                            </span>
                                                                        </li>)}
                                                                    </ol>
                                                                </div>
                                    ...
                                    <div role="status" className='flex justify-center mt-6 '>
                                        <svg aria-hidden="true" className="mr-2 w-8 h-8 text-gray-200 animate-bounce dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
