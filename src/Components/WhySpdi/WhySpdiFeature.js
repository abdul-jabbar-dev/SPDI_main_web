...
const SingleSql = ({whatwedo}) => {
    return (
        <li className="flex">
            <div className="px-4 text-5xl font-extralight text-indigo-700">{whatwedo._id+1}.</div>
            <div>
                <div className="text-xl font-bold text-indigo-800">{whatwedo.title}</div>
                <p className="max-w-xs py-2 text-sm text-indigo-900">{whatwedo.discreption}</p>
            </div>
        </li>
    )
}
